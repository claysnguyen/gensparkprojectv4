const c=document.querySelectorAll(".broker-checkbox"),f=document.getElementById("clearSelection"),o=document.getElementById("emptyState"),l=document.getElementById("comparisonTable"),r=document.getElementById("brokerHeaders"),t={rating:document.getElementById("row-rating"),hq:document.getElementById("row-hq"),established:document.getElementById("row-established"),regulation:document.getElementById("row-regulation"),fees:document.getElementById("row-fees"),platforms:document.getElementById("row-platforms"),regulations:document.getElementById("row-regulations"),spreads:document.getElementById("row-spreads")},i={};async function p(){(await(await fetch("/content/brokers.json")).json()).brokers.forEach(e=>{i[e.slug]=e})}p();function d(){const n=Array.from(c).filter(s=>s.checked).map(s=>s.value);if(n.length<2){o?.classList.remove("hidden"),l?.classList.add("hidden");return}o?.classList.add("hidden"),l?.classList.remove("hidden"),r&&(r.innerHTML='<div class="p-4 font-medium text-gray-500">Feature</div>',n.forEach(s=>{const e=i[s];e&&(r.innerHTML+=`
          ...
          `)})),n.forEach(s=>{const e=i[s];if(e&&(t.rating&&(t.rating.innerHTML+=`
        ...
        `),t.platforms&&(t.platforms.innerHTML+=`
          <div class="text-center">
            <div class="flex flex-wrap justify-center gap-1">
              ${(e.platforms||[]).slice(0,2).map(a=>`<span class="text-xs bg-gray-100 px-2 py-1 rounded">${a}</span>`).join("")}
        ...
        `),t.regulations&&(t.regulations.innerHTML+=`
          <div class="text-center text-sm">
            ${(e.regulations||[]).map(a=>`<div class="text-xs">${a.jurisdiction}</div>`).join("")||"N/A"}
          </div>
        `),t.spreads)){const a=(e.spreads||[]).find(m=>m.currencyPair==="EUR/USD");t.spreads.innerHTML+=`
          <div class="text-center">
            <span class="font-semibold">${a?a.spreadFrom:"N/A"}</span>
            <span class="text-gray-400 text-sm"> pips</span>
          </div>
        `}})}c.forEach(n=>{n.addEventListener("change",()=>{Object.values(t).forEach(s=>{s&&(s.innerHTML="")}),d()})});f?.addEventListener("click",()=>{c.forEach(n=>n.checked=!1),Object.values(t).forEach(n=>{n&&(n.innerHTML="")}),d()});
